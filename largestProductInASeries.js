...
//similar to bubble sort, split the array into n sized pieces, find product, sort largest
const largestProduct = (n) => {
  //set index to n less than end of array, to avoid going over the array
  let start = 0;
  let end = n;
  let products = [];
  while (start <= thousandDigits.length - n) {
    let productArr = thousandDigits.slice(start, end).reduce((a, b) => a * b);
